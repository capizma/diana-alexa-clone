        ...
        if when_pos != 0 and " to " in text:
            if when_pos < text.find(" to "):
                ...
                if len(text[text.find(" to ") : when_pos].replace(" to ", "")) > 0:
                    desc_context = text[text.find(" to ") : when_pos].replace(
                        " to ", ""
                    )
...
def parse_timer(text, out):
    if "minute" in text and "hour" not in text:
        ...
        out = (
            "#TIMER_ADD_"
            + str((datetime.today() + timedelta(minutes=clean)).hour)
            + "#"
            + str((datetime.today() + timedelta(minutes=clean)).minute)
        )
    ...
    if "hour" in text and "minute" not in text:
        ...
        out = (
            "#TIMER_ADD_"
            + str((datetime.today() + timedelta(hours=clean)).hour)
            + "#"
            + str((datetime.today() + timedelta(hours=clean)).minute)
        )
