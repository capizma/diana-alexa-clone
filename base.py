...
def evoke_response(context_key):
    ...
    if "#WIKIPEDIA" in context_key:
        try:
            return wikipedia.summary(
                context_key.replace("#WIKIPEDIA_", ""), auto_suggest=False, sentences=2
            )
        except:
            return (
                "Sorry, I can't find anything on Wikipedia for "
                + context_key.replace("#WIKIPEDIA_", "")
            )
    if "#WHAT_SONG" in context_key:
        current = sp.current_user_playing_track()

        return (
            "The song that's currently playing is "
            + current["item"]["name"]
            + " by "
    ...
    if "#REMINDER_UPDATE" in context_key:
        ...
        if context_key == "#REMINDER_WEEK":
            subset = df[
                (df["ds"] >= dt_today) & (df["ds"] <= dt_today + timedelta(days=7))
            ]
            n_reminders = len(subset)
            period = "this week"
        ...
        if context_key == "#REMINDER_NEXTWEEK":
            subset = df[
                (df["ds"] >= dt_today + timedelta(days=7))
                & (df["ds"] <= dt_today + timedelta(days=14))
            ]
            n_reminders = len(subset)
            period = "next week"
        ...
        if n_reminders > 0:
            out_text = (
                "You have " + str(n_reminders) + " scheduled for " + period + ". "
            )
    ...
    if "#TIME" in context_key:
        ...
        if dt_hour > 12:
            out_text = (
                "The time is now " + str(dt_hour - 12) + " " + dt_minute_alt + " P M"
            )
    ...
    if "#DATE" in context_key:
        ...
        out_text = (
            ...
            + suffix
            + " of "
            + datetime.strftime(dt_today, "%B")
            + " "
            + datetime.strftime(dt_today, "%Y")
        )
    ...
    if "#DEFINE" in context_key:
        try:
            out_text = (
                str(dictionary.meaning(context_key.replace("#DEFINE_", "")))
                .replace("{", "")
                .replace("}", "")
                .replace("[", "")
                .replace("]", "")
            ...
            return (
                ...
                + context_key.replace("#DEFINE_", "")
                + ". Try asking for something less niche or using only one word."
            )
    ...
    if "#NEXT_HOLIDAY" in context_key:
        ...
        return (
            ...
            + suffix
            + " of "
            + datetime.strftime(hols[0], "%B")
            + " "
            + datetime.strftime(hols[0], "%Y")
        )
...
def context_stream(sp):
    ...
    if "#REMINDER_DELETE" in context:
        say_tts(
            "This will delete all reminders. Say yes to continue this action", False
        )
    ...
    if "_NONE" in context:
        ...
        if "#TIMER" in context:
            ...
            while break_chase == False:
                ...
                alt_context = parse_utils.parse_timer(text, "")
                ...
                if "#REPEAT" in alt_context and break_chase == False:
                    say_tts(
                        "How many minutes would you like to set your timer for?", False
                    )
                elif break_chase == False:
                    say_tts(
                        "Sorry, I didn't get that. How many minutes do you want?", False
                    )
        ...
        if "#REMINDER" in context:
            ...
            if "#REMINDER_UPDATE_NONE" in context:
                ...
                while break_chase == False:
                    ...
                    alt_context = parse_utils.parse_calendar(" " + text, "")
                    ...
                    if "cancel" in text or "exit" in text or "abort" in text:
                        ...
                        "#REMINDER_UPDATE_NONE" not in alt_context
                        and "#REMINDER_UPDATE" in alt_context
                    ):
                    ...
                    if "#REPEAT" in alt_context and break_chase == False:
                        ...
                        say_tts(
                            "Sorry, I didn't get that. Which day would this be?", False
                        )
            ...
            if disregard == False:
                if context[-5:] == "_NONE":
                    ...
                    while break_chase == False:
                        ...
                        alt_context = parse_utils.parse_calendar(
                            "remind me to " + text + " on the 1st january", ""
                        )
                        ...
                        if "#REPEAT" in alt_context and break_chase == False:
                            ...
                            say_tts(
                                "Sorry, I didn't get that. What is the title?", False
                            )
...
if __name__ == "__main__":
    ...
    kern_post.bootstrap(learnFiles=["aiml/std.aiml", "aiml/rules_post.aiml"])
        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                try:
                    ...
                    if (
                        "diana" in text.lower()
                        or "anna" in text.lower()
                        or "dinosaurus" in text.lower()
                    ):
                        paused = False
                        ...
                        if "#PLAY" in result[1]:
                            try:
                                sp.start_playback(
                                    context_uri=result[1].replace("#PLAY_", "")
                                )
                            except:
                                say_tts(
                                    "I would love to, but your Spotify is unavailable :(",
                                    True,
                                )
                        if "#NEXT_SONG" in result[1]:
                            ...
                            paused == False and "#PAUSE" not in result[1]
                        ) or "#UNPAUSE" in result[1]:
                            sp.start_playback()
